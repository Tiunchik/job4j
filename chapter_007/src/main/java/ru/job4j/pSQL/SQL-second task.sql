...
insert into product (nameProd, type_id, expired_date, price) values
('Мороженное мясо',
...
select * from print
where expired_date < (date_trunc('month',now()) + interval '1 month');
...
select typeProd.nameType from typeprod, product where product.type_id = typeProd.id_type group by typeProd.id_type having count(typeProd.id_type) < 10;


select typeprod.nameType from typeprod where
id_type = (select product.type_id from product group by product.type_id having count(product.type_id) < 10);
